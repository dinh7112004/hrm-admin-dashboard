        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8 relative max-w-[1600px] mx-auto pb-10">
            ...
            <div className={`relative overflow-hidden rounded-[24px] p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 transition-all duration-500 ${totalPending > 0 ? 'bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 shadow-2xl shadow-blue-500/20' : 'bg-white border border-slate-200 shadow-sm'}`}>
                ...
                <div className="relative z-10">
                    <h3 className={`text-2xl font-black flex items-center gap-3 tracking-tight ${totalPending > 0 ? 'text-white' : 'text-slate-800'}`}>
                        {totalPending > 0 ? (
                ...
                {totalPending > 0 && (
                    <button onClick={handleApproveAll} className="relative z-10 bg-white text-blue-700 text-sm px-8 py-3.5 rounded-2xl font-black shadow-xl hover:shadow-2xl hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all duration-300 w-full md:w-auto flex justify-center items-center gap-2">
            ...
            <div className="bg-white border border-slate-200 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col">
                <div className="px-8 py-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/50 backdrop-blur-xl z-10">
                    <h3 className="font-black text-slate-800 text-xl tracking-tight flex items-center gap-3 uppercase">
                        <MapPin className="text-blue-500" />
                        Nhật ký Điểm danh
                    </h3>

                    <div className="flex items-center gap-3 bg-slate-50/80 p-1.5 rounded-2xl border border-slate-200/60 shadow-inner">
                        <div className="bg-white px-4 py-2 rounded-xl border border-slate-100 shadow-sm flex items-center gap-2">
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Ngày:</label>
                            <input
                                type="date"
                        ...
                        {filterDate && (
                            <button
                                ...
                                className="text-xs font-bold text-rose-500 hover:text-white hover:bg-rose-500 px-4 py-2.5 rounded-xl transition-all duration-300 uppercase tracking-wider"
                            >
                                Bỏ lọc
                            </button>
                        )}
                ...
                <div className="overflow-x-auto p-4 md:p-8 pt-4">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-[11px] uppercase text-slate-400 font-black tracking-widest border-b border-slate-100">
                                    ...
                                    <tr key={item._id || idx} className="group hover:bg-slate-50/80 transition-colors duration-200">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-3">
                                                <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-blue-100 to-indigo-50 border border-blue-200 flex items-center justify-center text-blue-700 font-black text-sm shadow-sm group-hover:scale-110 transition-transform">
                                        ...
                                        <td className="px-6 py-5">
                                            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-full border ${item.type === 'REMOTE' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                                                <MapPin size={12} />
                                                {item.type === 'REMOTE' ? 'Làm Online' : 'Tại Công Ty'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className={`inline-flex items-center gap-1 text-[11px] font-black px-3 py-1.5 rounded-full uppercase tracking-wider 
                                        ...
                                        <td className="px-6 py-5 text-right">
                                            <button
                                                ...
                                                className="inline-flex p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-300 hover:shadow-sm"
                                            >
                                ...
                                <tr>
                                    <td colSpan={5} className="py-16">
                                        <div className="flex flex-col items-center justify-center text-slate-400 animate-in fade-in zoom-in-95 duration-500">
                                            ...
                                            <p className="font-bold text-slate-600 text-lg">Chưa có dữ liệu</p>
                                            <p className="text-sm mt-1 text-slate-400">Không có ai điểm danh {filterDate ? `trong ngày ${filterDate.split('-').reverse().join('/')}` : 'nào'}.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            {selectedRecord && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-400">
                        <div className="bg-gradient-to-b from-slate-50 to-white p-6 border-b border-slate-100 flex justify-between items-center">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                                <div className="h-14 w-14 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 font-black text-xl shadow-inner">
                                    {(selectedRecord.userId?.name || selectedRecord.userId?.phone || "N").charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Nhân viên</p>
                                    <p className="font-black text-slate-800 text-lg">{selectedRecord.userId?.name || selectedRecord.userId?.phone}</p>
                                </div>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <div className="relative pl-10">
                                    ...
                                    <span className="text-amber-600 text-[11px] font-black uppercase tracking-widest block mb-1">Ghi chú lúc Check-in</span>
                                    <div className="bg-amber-50/50 border border-amber-100 p-4 rounded-2xl text-amber-900 text-sm font-medium leading-relaxed">
                            ...
                            {selectedRecord.status === 'APPROVED' ? (
                                <div className="bg-emerald-50/50 border border-emerald-100 p-4 rounded-2xl text-emerald-900 text-sm font-bold text-center">
                                    <CheckCircle size={16} className="inline mr-2 text-emerald-500" />
                                    Đã được duyệt.
                                </div>
                            ) : selectedRecord.status === 'REJECTED' ? (
                                <div className="bg-rose-50/50 border border-rose-100 p-4 rounded-2xl text-rose-900 text-sm font-bold text-center">
                                    <X size={16} className="inline mr-2 text-rose-500" />
                                    Đã bị từ chối.
                                </div>
                            ) : (
                                <div className="pt-2">
                                    <label className="text-slate-500 text-[11px] font-black uppercase tracking-widest mb-2 flex items-center gap-2">
                                        <MessageSquare size={14} /> Phản hồi cho nhân viên
                                    </label>
                                    <textarea
                                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-medium text-slate-700 focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all resize-none shadow-inner"
                        ...
                        <div className="p-6 pt-0 flex gap-3 bg-white">
                            <button onClick={() => setSelectedRecord(null)} className="flex-1 py-4 rounded-2xl font-bold text-slate-500 bg-slate-50 hover:bg-slate-100 transition-all">
                                Đóng
                            </button>
                            ...
                            {selectedRecord.status === 'PENDING' && (
                                <button
                                    ...
                                    className="flex-[1.5] py-4 rounded-2xl font-black text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/30 flex justify-center items-center gap-2 transition-all hover:-translate-y-0.5"
                                >
            ...
            {activeListType && modalConfig && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 slide-in-from-bottom-4 duration-400">
                        <div className={`p-8 pb-6 border-b flex justify-between items-center bg-gradient-to-br from-${modalConfig.color}-50/50 to-white border-${modalConfig.color}-100`}>
                            <div>
                                <h3 className={`font-black text-${modalConfig.color}-800 text-2xl flex items-center gap-3 tracking-tight`}>
                                    <div className={`p-3 bg-${modalConfig.color}-100 text-${modalConfig.color}-600 rounded-2xl shadow-sm`}>
                                        {modalConfig.icon}
                                    </div>
                                    {modalConfig.title}
                                </h3>
                                {activeListType !== 'TOTAL' && filterDate && (
                                    <p className={`mt-2 text-sm font-medium text-${modalConfig.color}-600/70`}>
                            ...
                            {modalConfig.data.length > 0 ? (
                                <ul className="space-y-3">
                                    {modalConfig.data.map((item: any, idx: number) => {
                                        ...
                                        return (
                                            <li key={item._id || idx} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-[20px] shadow-sm hover:shadow-md hover:border-slate-200 transition-all duration-300 group">
                                                <div className="flex items-center gap-4">
                                                    <div className={`h-12 w-12 rounded-full bg-${modalConfig.color}-50 border border-${modalConfig.color}-100 flex items-center justify-center text-${modalConfig.color}-600 font-black text-lg group-hover:scale-110 transition-transform`}>
                                                        {name.charAt(0).toUpperCase()}
                                                    </div>
                                                    <div>
                                                        <p className="font-bold text-slate-800 text-base">{name}</p>
                                                        {activeListType === 'LEAVE' ? (
                                                            <p className="text-purple-600 text-[11px] font-bold mt-0.5">{item.leaveType} | {item.startDate}</p>
                                                        ) : (
                                                            <p className="text-slate-400 text-xs font-medium mt-0.5">{phone}</p>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                {activeListType === 'LEAVE' && (
                                                    <div className="text-right">
                                                        <span className={`inline-flex items-center text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-wider border ${item.status === 'APPROVED' ? 'text-emerald-600 bg-emerald-50 border-emerald-200' : item.status === 'REJECTED' ? 'text-rose-600 bg-rose-50 border-rose-200' : 'text-amber-600 bg-amber-50 border-amber-200'}`}>
                                                            {item.status}
                                                        </span>
                                                    </div>
                                                )}
                                                ...
                                                {!modalConfig.isUserOnly && activeListType !== 'LEAVE' && item.checkInTime && (
                                                    <div className="text-right bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
                                                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Giờ vào</span>
                                                        <p className={`text-sm font-black ${activeListType === 'LATE' ? 'text-rose-600' : 'text-emerald-600'}`}>
                                ...
                                <div className="text-center py-12 flex flex-col items-center">
                                    ...
                                    <p className="font-bold text-slate-600 text-lg">Danh sách trống</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-6 bg-white border-t border-slate-100">
                            <button
                                ...
                                className={`w-full py-4 rounded-2xl font-bold text-${modalConfig.color}-700 bg-${modalConfig.color}-50 hover:bg-${modalConfig.color}-100 transition-all`}
                            >
                                Đóng lại
                            </button>
                        </div>
